...
#%% import del18O data and time, scale it,  and plot
...
#  plot del18O and scaled sea level vs time
...
plt.title('initial topography & modern sea level')
...
#    topo[:,i] = topo[:,i-1] + (U[i] * dt) # uplift topography for this time step and all space
...
for i in range(0,len(time)-1):
    depth = SL[i] - topo[:,i] # calculate ocean depth at this time for all space
    coral[:,i] = coral[:,i-1] + (coralgrowth.coralGrowthRate(depth) * dt[i]) # grow coral for this time and all space
    zb[:,i+1] = zb[:,i] + (U[i] * (time[i]-time[i-1])) # calculate uplifted bedrock for next time
...
for i in range(0,len(time)-1):
    if i % plot == 0:
        ...
        ax1.set_xlim(0,xmax)
        ax1.set_ylim(0,np.max(coral)+5)
        ax1.text(100,22,str(np.int(time[i]))+' years')
